...
| 组件 | 版本 |
|---|---|
| DeepSeek Harness | `dsh-v0.1.7-alpha.1`（peer 范围已接纳 alpha.2 线：`>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0 \|\| >=0.1.6-0 <0.2.0`）。该线上 `Session.append` 的第三参仅对表面事件类型为 `SurfaceIntent`，因此 `fund-research/*` 审计事件仍不落盘（工具结果与已封存快照/报告即审计轨迹）。已于 2026-09-18 核验（双 typecheck 尺子 + 176 项测试）。 |
