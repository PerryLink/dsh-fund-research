...
- 会话事件为仅日志审计记录；钉版的 0.1.0-rc.6 peers 没有 `ignorable` 信封，因此由*未安装*本插件的 build 恢复会话时会拒绝这些日志行——与本家族其他研究插件接受的同一折衷。
...
测试使用来自 0.1.0-rc.6 peers 的真实 `Context`/`SessionStore`/`ToolRuntime`/`LocalJobRegistry`/存储接缝；网络仅在 fetch 边界由保存的真实响应 fixtures（`fixtures/`，基金 161725）替换。用 `.tmp/` 下的采集脚本刷新 fixtures。
...
PerryLink 独立 DeepSeek Harness 插件家族成员，共享同一工程基线：钉版 0.1.0-rc.6 peers、响亮失败的 Schemastery 配置、五语 README、真实接缝 vitest 覆盖。
