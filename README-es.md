...
[![dshfind](https://dshfind.com/api/badge/PerryLink/dsh-fund-research?metric=downloads&lang=es)](https://dshfind.com/es/plugins/PerryLink/dsh-fund-research?ref=badge)
...
| Componente | Versión |
|---|---|
| DeepSeek Harness | `dsh-v0.1.7-rc.1` (el rango de peers admite la línea alpha.2: `>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0 \|\| >=0.1.6-0 <0.2.0 \|\| >=0.1.7-0 <0.2.0`). En esta línea el tercer parámetro de `Session.append` es un `SurfaceIntent` solo para tipos de superficie, así que los eventos de auditoría `fund-research/*` siguen sin registrarse (los resultados de herramientas y el snapshot/informe sellado son el rastro de auditoría). Verificado el 2026-09-24 (doble typecheck + 176 pruebas). |
...
- **Huecos honestos** — una fuente fallida o degradada produce una declaración explícita de hueco de datos en la sección afectada. El plugin nunca rellena un hueco con una cifra inventada.
...
- **Eventos de auditoría de sesión (según el host)** — los eventos solo-registro `fund-research/snapshot` y `fund-research/report` llevan el código, el directorio de versión, el hash del manifiesto y la lista de huecos (visible para el modelo ⟺ registrado) *cuando el host admite tipos de evento fuera del repositorio*; en hosts `0.1.2-alpha.1`–`0.1.5-alpha.1` el catálogo de tipos conocidos se genera en el build dentro del repositorio, así que la puerta no añade nada y los resultados de las herramientas más los artefactos sellados son la pista de auditoría.
...
Reinicia el perfil tras instalar (la activación del bundle es por reinicio). Los perfiles incluidos componen la pila de almacenamiento mediante `dsh-base` (`dsh-storage` + `dsh-storage-json` + `dsh-storage-domain`); el parche del bundle monta solo la fila del plugin.
...
Las pruebas usan los seams REALES `Context`/`SessionStore`/`ToolRuntime`/`LocalJobRegistry`/almacenamiento de los peers 0.1.7-rc.1; la red se reemplaza solo en la frontera de fetch por fixtures de respuestas reales guardadas (`fixtures/`, fondo 161725). Refresca los fixtures con los scripts de `.tmp/`.
