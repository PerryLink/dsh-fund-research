...
- Los eventos de sesión son registros de auditoría solo-registro; los peers 0.1.0-rc.6 fijados no ofrecen envoltura `ignorable`, así que una sesión restaurada por un build *sin* este plugin rechaza esas líneas de registro — la misma compensación aceptada por otros plugins de investigación de esta familia.
...
pnpm test                                     # 113 pruebas sobre seams reales
...
Las pruebas usan los seams REALES `Context`/`SessionStore`/`ToolRuntime`/`LocalJobRegistry`/almacenamiento de los peers 0.1.0-rc.6; la red se reemplaza solo en la frontera de fetch por fixtures de respuestas reales guardadas (`fixtures/`, fondo 161725). Refresca los fixtures con los scripts de `.tmp/`.
...
Construido por los contribuidores de dsh-fund-research. Issues y pull requests bienvenidos en el repositorio indicado arriba.

## PerryLink DSH Plugin Family

Parte de una familia de plugins independientes de DeepSeek Harness que comparten una base de ingeniería: peers 0.1.0-rc.6 fijados, configuración Schemastery de fallo ruidoso, READMEs en cinco idiomas y cobertura vitest sobre seams reales.
